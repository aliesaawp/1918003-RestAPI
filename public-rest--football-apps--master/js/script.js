const ApiKey = "a85e5a69930d4305837a201eceea84c6";
...
function showTeamInfo(id){
    ...
    // contents.innerHTML = "JANGAN KEPO " + url;
    fetch(url, fetchHeader)
        .then(response => response.json())
        .then(detailtim => {
            console.log(detailtim);
            let detail = `
                    <img src="${detailtim.crestUrl}" alt="${detailtim.name}" width="100px"
                <div class="card-content">
                    ...
                    <p> Berdiri: ${detailtim.founded} <br>
                        Stadion: ${detailtim.venue} <br>
                        Negara : ${detailtim.area.name} <br>
                        Alamat : ${detailtim.address}
                    </p>
                </div>
            `;
